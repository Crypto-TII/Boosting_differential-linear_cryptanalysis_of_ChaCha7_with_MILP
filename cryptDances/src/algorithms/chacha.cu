...
#define CHACHAQUARTERROUND_PACK(x, a, b, c, d) \
 asm ("add.u32 %0, %0, %1; \n\t" \
 "xor.b32 %3, %3, %0; \n\t" \
...
#define HALF_1_CHACHAQUARTERROUND(x, a, b, c, d) \
 asm ("add.u32 %0, %0, %1; \n\t" \
 "xor.b32 %3, %3, %0; \n\t" \
 "shf.l.clamp.b32 %3, %3, %3, %4; \n\t" \
 "add.u32 %2, %2, %3; \n\t" \
...
#define HALF_2_CHACHAQUARTERROUND(x, a, b, c, d) \
 asm ("add.u32 %0, %0, %1; \n\t" \
 "xor.b32 %3, %3, %0; \n\t" \
 "shf.l.clamp.b32 %3, %3, %3, %4; \n\t" \
 "add.u32 %2, %2, %3; \n\t" \
...
__device__ void CHACHAQUARTERROUND(uint32_t x[16], int a, int b, int c, int d) {
    HALF_1_CHACHAQUARTERROUND(x, a, b, c, d)
    HALF_2_CHACHAQUARTERROUND(x, a, b, c, d)
}
  ...
  __host__ __device__ void chacha_init(uint32_t state[STATE_SIZE], uint32_t k[KEY_SIZE], uint32_t nonce[NONCE_SIZE], uint32_t ctr[CTR_SIZE])
{
 ...
 __device__ void chacha_odd_round(uint32_t x[STATE_SIZE])
{
  ...
  __device__ void chacha_even_round(uint32_t x[STATE_SIZE])
{
...
__device__ void chacha_rounds(uint32_t state[STATE_SIZE], uint32_t rounds, uint32_t last_round)
{
    uint32_t i;
  ...
  __host__ __device__ void chacha_invert_rounds(uint32_t state[STATE_SIZE], uint32_t rounds, uint32_t last_round)
{
 ...
 __device__ void chacha_subrounds(uint32_t state[STATE_SIZE], uint32_t subrounds, uint32_t last_subround)
{
    uint32_t i, rounds;
    ...
    if((last_subround%CHACHA_TOTAL_OF_DIFFERENT_SUBROUNDS) != 0)
    {
        ...
        {
            case 1:
                HALF_2_CHACHAQUARTERROUND(state, 0, 4, 8, 12)
                HALF_2_CHACHAQUARTERROUND(state, 1, 5, 9, 13)
                HALF_2_CHACHAQUARTERROUND(state, 2, 6, 10, 14)
                HALF_2_CHACHAQUARTERROUND(state, 3, 7, 11, 15)
                subrounds--;
                if(subrounds == 0) return;
            case 2:
                HALF_1_CHACHAQUARTERROUND(state, 0, 5, 10, 15)
                HALF_1_CHACHAQUARTERROUND(state, 1, 6, 11, 12)
                HALF_1_CHACHAQUARTERROUND(state, 2, 7, 8, 13)
                HALF_1_CHACHAQUARTERROUND(state, 3, 4, 9, 14)
                subrounds--;
                if(subrounds == 0) return;
            case 3:
                HALF_2_CHACHAQUARTERROUND(state, 0, 5, 10, 15)
                HALF_2_CHACHAQUARTERROUND(state, 1, 6, 11, 12)
                HALF_2_CHACHAQUARTERROUND(state, 2, 7, 8, 13)
                HALF_2_CHACHAQUARTERROUND(state, 3, 4, 9, 14)
                subrounds--;
                if(subrounds == 0) return;
        }
    }
    ...
    if(subrounds==0)return;
            HALF_1_CHACHAQUARTERROUND(state, 0, 4, 8, 12)
            HALF_1_CHACHAQUARTERROUND(state, 1, 5, 9, 13)
            HALF_1_CHACHAQUARTERROUND(state, 2, 6, 10, 14)
            HALF_1_CHACHAQUARTERROUND(state, 3, 7, 11, 15)
 ...
 __host__ __device__ void chacha_invert_subrounds(uint32_t state[STATE_SIZE], uint32_t subrounds, uint32_t last_subround)
{
 ...
 __device__ void chacha_encrypt_rounds(uint32_t output[STATE_SIZE], uint32_t input[STATE_SIZE], uint32_t rounds)
{
    ...
    uint32_t intermediate[STATE_SIZE], uint32_t rounds, uint32_t last_round)
{
  ...
  __device__ void chacha_encrypt_subrounds(uint32_t output[STATE_SIZE], uint32_t input[STATE_SIZE], uint32_t subrounds)
{
    uint32_t x[STATE_SIZE];
    uint32_t i;
