...
__host__ __device__ void xor_array_dist2(uint32_t *z, uint32_t *x, uint32_t *y, int size, uint32_t *non_zero_words, uint32_t *non_zero_bits, int size_non_zero_bits)
{

    for (uint8_t i=0;i<size_non_zero_bits;i++)
        z[non_zero_words[i]] = x[non_zero_words[i]] ^ y[non_zero_words[i]];
...
/*__device__ uint8_t xor_bits_of_state(uint32_t state[STATE_SIZE])
{

    uint32_t x = state[2]^state[13];//^state[7];//
    x = x ^ (x >> 1);
    x = (x ^ (x >> 2)) & 0x11111111;
    x = x*0x11111111;
    return (x >> 28) & 1;
...
__device__ uint8_t xor_bits_of_state_dist2(uint32_t state[STATE_SIZE], uint32_t *non_zero_words, uint32_t *non_zero_bits, int size_non_zero_bits)
{

    uint32_t x = 0;//state[2]^state[13];//^state[7];//

    for(uint8_t i=0;i<size_non_zero_bits;i++)
        x ^= state[non_zero_words[i]];
...
__device__ uint8_t check_parity_of_equation_dist2(uint32_t state[STATE_SIZE], uint32_t ODmask[STATE_SIZE], uint32_t *non_zero_words, uint32_t *non_zero_bits, int size_non_zero_bits)
{
    uint32_t aux[STATE_SIZE] = {0};
    for (uint8_t i = 0; i< size_non_zero_bits;i++)
        aux[non_zero_words[i]] = state[non_zero_words[i]] & ODmask[non_zero_words[i]];

    return(xor_bits_of_state_dist2(aux, non_zero_words, non_zero_bits, size_non_zero_bits));
}

__device__ uint8_t check_parity_of_equation(uint32_t state[STATE_SIZE], uint32_t ODmask[STATE_SIZE])
{
...
__device__ uint8_t check_parity_of_linear_relation(uint32_t input_mask[STATE_SIZE],
