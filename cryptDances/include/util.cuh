...
#define GENERATE_RANDOM_STATE_dist2(state, alt_state)\
    state[0] = curand(&rng);\
    state[1] = curand(&rng);\
    state[2] = curand(&rng);\
    state[3] = curand(&rng);\
    ...
    alt_state[8]  = state[8];\
    alt_state[9]  = state[9];\
    alt_state[10] = state[10];\
    alt_state[11] = state[11];\
    alt_state[12] = state[12];\
    alt_state[13] = state[13];\
    alt_state[14] = state[14];\
    alt_state[15] = state[15];\
...
__device__ void and_array(uint32_t *z, uint32_t *x, uint32_t *y, int size);
...
__device__ uint8_t check_parity_of_linear_relation(uint32_t inputMask[STATE_SIZE],
